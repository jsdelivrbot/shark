            ...
            <el-tabs v-model="activeName">
                <el-tab-pane label="新增用户" name="0">
                    <el-collapse v-model="newUserName">
                        <el-collapse-item title="概况" name="0">
                            <ele-form :config="newUserSurvey_config" v-on:receive="newUserSurvey_submit"
                                      :defaultdata="newUserSurveyHtml"></ele-form>
                            <table-option :parent-message="newUserSurvey_Msg" v-loading="loading1"
                                          element-loading-text="拼命加载中"></table-option>
                        </el-collapse-item>
                        <el-collapse-item title="新增列表" name="1">
                            <ele-form :config="new_user_config" v-on:receive="new_user_submit"
                                      :defaultdata="newUserHtml"></ele-form>
                            <table-option :parent-message="newUser_Msg" v-on:message="newUserMessage"
                                          v-on:pagedata="newUserChangePage" v-loading="loading2"
                                          element-loading-text="拼命加载中"></table-option>
                        </el-collapse-item>
                    </el-collapse>
                </el-tab-pane>
                <el-tab-pane label="活跃用户" name="1">
                    <el-collapse v-model="acUserName">
                        <el-collapse-item title="概况" name="0">
                            <ele-form :config="acUserSurvey_config" v-on:receive="acUserSurvey_submit"
                                      :defaultdata="acUserSurveyHtml"></ele-form>
                            <table-option :parent-message="acUserSurvey_Msg" v-loading="loading3"
                                          element-loading-text="拼命加载中"></table-option>
                        </el-collapse-item>
                        <el-collapse-item title="活跃列表" name="1">
                            <ele-form :config="active_config" v-on:receive="active_submit"
                                      :defaultdata="activeHtml"></ele-form>
                            <table-option :parent-message="active_Msg" v-on:pagedata="activeUserChangePage"
                                          v-on:message="activeMessage" v-loading="loading4"
                                          element-loading-text="拼命加载中"></table-option>
                        </el-collapse-item>
                    </el-collapse>
                </el-tab-pane>
                <el-tab-pane label="留存用户" name="2">
                    <ele-form :config="keep_user_config" v-on:receive="keep_user_submit"
                              :defaultdata="keepUserHtml"></ele-form>
                    <table-option :parent-message="keep_Msg" v-on:pagedata="keepUserChangePage"
                                  v-on:message="keepMessage" v-loading="loading5"
                                  element-loading-text="拼命加载中"></table-option>
                </el-tab-pane>
            </el-tabs>
            <br/>
        ...
        methods: {
            ...
            acUserSurvey_submit(arg) {
                ...
                _self.$res.postData(_self, '/Useranalysis/active_user_statistics_info/', arg[0]).then((res) => {
                    ...
                    _self.acUserSurvey_Msg.data = res.reverse();
