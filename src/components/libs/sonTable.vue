            ...
            <el-table-column v-for="col in tblCols" :key="col.name" :prop="col.name" :label="col.title" :sortable="col.sortable" :formatter="col.formatter">
                <template scope="scope">
                    <div v-if="col.template">
                        <el-button v-for="btn in col.template.btns" size="small" :key="btn.name" :type="btn.type" @click="outPutRow(scope.$index, scope.row, btn.funcName, scope)">{{btn.name}}</el-button>
                    </div>
                ...
                <el-table-column v-for="sub_col in col.sub" :key="sub_col.name" :prop="sub_col.name" :label="sub_col.title" :sortable="sub_col.sortable" :formatter="sub_col.formatter">
                    <template scope="scope">
                        <div v-if="sub_col.template">
                            <el-button v-for="btn in sub_col.template.btns" size="small" :key="btn.name" :type="btn.type" @click="outPutRow(scope.$index, scope.row,btn.funcName,scope)">{{btn.name}}</el-button>
                        </div>
