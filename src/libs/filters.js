...
export function transToDate(cellValue) {
    return isNaN(cellValue) ? (cellValue || '') : (cellValue ? convertIntToTime(cellValue).slice(0, 10) : '');
}
//unix时间戳转换为准确时间
export function transToTime(cellValue) {
    return isNaN(cellValue) ? (cellValue || '') : (cellValue ? convertIntToTime(cellValue) : '');
}
...
export function bigNumberFormatter(cellValue) {
    return cellValue ? (cellValue + '').replace(/\d(?=(\d{3})+(\.\d{1,2})?$)/g, ($1) => { return $1 + ',' }) : 0;
}
